...
log = logging.getLogger("anima.api.study")
...
def study_status(user: User = Depends(current_user)) -> dict:
    ...
    import anima.db as db
    from anima.study.config import get_study_day, get_config
...
def study_report(user: User = Depends(current_user)) -> dict:
    ...
    from anima.study.tracker import get_full_report
...
def study_daily(limit: int = 30, user: User = Depends(current_user)) -> dict:
    ...
    from anima.study.tracker import get_daily_history
