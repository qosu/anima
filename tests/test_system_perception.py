...
class TestSystemPerceptionExclusion:
    ...
    def test_worktree_root_excluded(self, entity_user):
        ...
        path = os.path.join(settings.worktree_root, "some-branch", "anima", "fix.py")
    ...
    def test_pycache_excluded(self, entity_user):
        ...
        path = os.path.join(
            settings.rawos_source_root, "anima", "__pycache__", "config.cpython-312.pyc"
        )
        handler = _SystemHandler(debounce_s=0.0)
        handler._handle(path, deleted=False)
        assert _system_fs_rows(path) == []
    ...
    def test_pyc_extension_excluded(self, entity_user):
        ...
        path = os.path.join(settings.rawos_source_root, "anima", "config.pyc")
        handler = _SystemHandler(debounce_s=0.0)
        handler._handle(path, deleted=False)
        assert _system_fs_rows(path) == []
...
class TestSystemPerceptionCapture:
    ...
    def test_source_modify_recorded(self, entity_user):
        """Modify event on anima source file → exactly 1 row in context_events."""
        path = os.path.join(settings.rawos_source_root, "anima", "config.py")
        handler = _SystemHandler(debounce_s=0.0)
        handler._handle(path, deleted=False)
    ...
    def test_source_type_is_system_fs(self, entity_user):
        ...
        path = os.path.join(settings.rawos_source_root, "anima", "scheduler", "proactive.py")
        handler = _SystemHandler(debounce_s=0.0)
        handler._handle(path, deleted=False)
    ...
    def test_entity_user_id_used_not_real_user(self, entity_user):
        ...
        path = os.path.join(settings.rawos_source_root, "anima", "models.py")
        handler = _SystemHandler(debounce_s=0.0)
        handler._handle(path, deleted=False)
        with db._conn() as conn:
            rows = conn.execute(
    ...
    def test_deleted_event_recorded(self, entity_user):
        ...
        path = os.path.join(settings.rawos_source_root, "anima", "context", "collector.py")
        handler = _SystemHandler(debounce_s=0.0)
...
class TestSystemPerceptionDebounce:
    ...
    def test_event_after_debounce_window_recorded(self, entity_user):
        ...
        path = os.path.join(settings.rawos_source_root, "anima", "api", "app.py")
        handler._handle(path, deleted=False)
        time.sleep(0.12)
        handler._handle(path, deleted=False)
    ...
    def test_different_paths_not_debounced_together(self, entity_user):
        ...
        handler = _SystemHandler(debounce_s=10.0)
        path_a = os.path.join(settings.rawos_source_root, "anima", "config.py")
        path_b = os.path.join(settings.rawos_source_root, "anima", "models.py")
...
class TestSystemPerceptionClassify:
    ...
    def test_systemd_unit_yields_config_change(self):
        path = "/etc/systemd/system/anima.service"
        event_type, _ = _classify(path, deleted=False)
        assert event_type == EVENT_TYPE_CONFIG_CHANGE
...
class TestSystemPerceptionIdentity:
    ...
    def test_multiple_events_all_under_entity(self, entity_user):
        ...
        handler = _SystemHandler(debounce_s=0.0)
        paths = [
            os.path.join(settings.rawos_source_root, "anima", "config.py"),
            os.path.join(settings.rawos_source_root, "anima", "models.py"),
            os.path.join(settings.rawos_source_root, "anima", "db", "__init__.py"),
        ]
        for p in paths:
