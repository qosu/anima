"""
kernel/tools — _targets_rawos_own_repo + _is_anima_source_tree use
...
- _is_anima_source_tree compares git --git-common-dir against _RAWOS_GIT_COMMON_DIR,
...
for _is_anima_source_tree, and patch settings directly for _targets_rawos_own_repo.
...
from anima.kernel.tools import _is_anima_source_tree, _targets_rawos_own_repo
...
def test_targets_rawos_own_repo_uses_settings_rawos_source_root():
    ...
    custom_root = "/custom/anima-install"
...
def test_is_anima_source_tree_uses_anima_git_common_dir(monkeypatch):
    """_RAWOS_GIT_COMMON_DIR is computed from settings.rawos_source_root + '/.git';
    _is_anima_source_tree uses this variable (same patching contract as tier
    enforcement tests — settable at runtime, not a buried literal)."""
    custom_git = "/custom/anima-install/.git"
    ...
    with patch("anima.kernel.tools.run_bash",
        ...
        result = asyncio.run(_is_anima_source_tree("/some/worktree"))
    assert result is True


def test_is_anima_source_tree_false_for_different_git_dir(monkeypatch):
    custom_git = "/custom/anima-install/.git"
    monkeypatch.setattr(tools, "_RAWOS_GIT_COMMON_DIR", custom_git)
...
def test_anima_git_common_dir_derived_from_settings_rawos_source_root():
