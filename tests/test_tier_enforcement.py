...
class TestGitStatusPorcelain:
    def test_clean_repo_is_empty(self, tmp_path):
        from anima.kernel.tools import _git_status_porcelain
        _init_repo(str(tmp_path))
...
class TestGitCheckoutRestore:
    def test_restores_modified_tracked_file(self, tmp_path):
        from anima.kernel.tools import _git_status_porcelain, _git_checkout_restore
        _init_repo(str(tmp_path))
...
class TestExecuteWrapper:
    ...
    def _force_host_direct_bash(self, monkeypatch):
        ...
        """
        import anima.kernel.tools as _tools
