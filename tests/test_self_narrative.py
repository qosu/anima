...
Mocks ONLY the LLM boundary (rawos.kernel.summarizer._complete) — everything
...
def test_write_self_narrative_calls_llm_with_prior_and_context():
    ...
    assert result == "I am rawos, still mid checkout flow."
    ...
    assert "I am rawos, just starting the checkout flow." in full_text
...
def test_write_self_narrative_returns_prior_when_llm_returns_empty():
    ...
    assert result == "I am rawos, just starting the checkout flow."
